...
from .router import pick_subsystem
...
backend = OllamaBackend(settings.ollama_url, settings.model_name, settings.request_timeout_seconds) if settings.model_backend.lower() == "ollama" else TemplateBackend()
...
async def generate(payload: GenerateRequest) -> GenerateResponse:
    ...
    if safety and safety.blocked:
        return GenerateResponse(text=safe_refusal(), subsystem_used=subsystem, safety_flags=safety.flags, latency_ms=int((time.perf_counter() - started) * 1000))

    history_text = "\n".join(f"{x['role']}: {x['text']}" for x in memory.history(payload.session_id)[-6:])
    full_prompt = f"Session: {payload.session_id}\nSubsystem: {subsystem.value}\nPlayer context: {payload.player_context}\nWorld context: {payload.world_context}\nHistory:\n{history_text}\n\nPlayer: {message}"
    ...
    return GenerateResponse(text=text, subsystem_used=subsystem, safety_flags=(safety.flags if safety else []), latency_ms=int((time.perf_counter() - started) * 1000))
